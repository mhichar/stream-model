...
def put_words_in_stream(conn, stream_name, csv, between_sec):
    '''
    ...
    for i in range(df.shape[0]):
        ...
        try:
            conn.put_record(stream_name, row, row[0])
            print("Put row: " + row + " into stream: " + stream_name)
        except Exception as e:
            sys.stderr.write("Encountered an exception while trying to put a word: "
                             + row + " into stream: " + stream_name + " exception was: " + str(e))
        time.sleep(between_sec)

def put_words_in_stream_periodically(conn, stream_name, csv, period_seconds):
    '''
    ...
    while True:
        put_words_in_stream(conn, stream_name, csv, period_seconds)
...
''')
    ...
    if args.period != None:
        ...
        put_words_in_stream(conn, stream_name, words, args.period)
